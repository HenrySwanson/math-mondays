...
* for each $d \ge 1$, add the sentence $\forall a_0 \forall a_1 \cdots \forall a_d \exists x \ a_0 + a_1 x + \cdots a_d x^d = 0$
...
We claim that $\ACF_0$ and $\ACF_p$ are complete theories. If that is indeed the case, then we can prove a stronger form of the Ax-Grothendieck theorem.
...
*Proof sketch*: We start by breaking our claim into a number of first-order sentences. For a fixed $d$, the sentence "for all polynomial maps $f$ of degree at most $d$, injectivity of $f$ implies surjectivity of $f$" can be expressed as a first-order sentence.
...
Since $\ACF_p$ can prove $\phi_d$, and $\ACF_0$ can prove $\phi_d$, we know that $\phi_d$ is true in $k$, no matter what the characteristic of $k$ is. And since this argument works for all $d$, this proves our theorem.

---

This proof is magical in two ways.

One is that, despite there being no homomorphisms between $\FFx{p}$ and $\CC$, we were able to somehow transport a claim between the two. This was possible not by looking at the structure of $\CC$ and $\FFx{p}$ themselves, but by using the structure of their axiomatizations. This is an example of the [compactness theorem](https://en.wikipedia.org/wiki/Compactness_theorem).
...
<ul>
<li>$\phi$ is true in some algebraically closed $k$ of characteristic $p$ for all but finitely many $p$</li>
<li>$\phi$ is true in some algebraically closed $k$ of characteristic $0$</li>
</ul>

Furthermore, if $\phi$ is true in some algebraically closed $k$, then it is true in all algebraically closed fields of that characteristic.
</div>

The first claim uses compactness, similar to our proof above, and the second comes from the fact that $\ACF_p$ and $\ACF_0$ are complete.
...
The proof is quite technical, and makes good use of the compactness theorem, but we won't cover it here. See [here](http://modeltheory.wikia.com/wiki/L%C3%B6wenheim-Skolem_Theorem) for a proof.
...
*Proof*: Let $p$ be a prime or $0$, and $\kappa$ be an uncountable cardinal. Let $K$ be an algebraically closed field of size $\kappa$ with characteristic $p$, and $B$ be a transcendence basis of $K$ over its prime subfield ($\FF_p$ or $\Bbb Q$). Because $\kappa$ is uncountable, a cardinality argument shows that $\\|B\\| = \kappa$. So, if $K'$ is another such field, and we pick a transcendence basis $B'$, it will also have cardinality $\kappa$. The bijection between $B$ and $B'$ induces an isomorphism between $k(B)$ and $k(B'). But since $K$ and $K'$ are algebraically closed, and algebraic over $k(B) \cong k(B')$, they are isomorphic!
...
TODO model theory definitions. Will fill out later.